...
COPY . .
RUN npm ci --omit=optional --no-audit && npm run lint && npm run build
...
WORKDIR /home/node/catenax-api
COPY --from=cleaner /home/node/catenax-api ./
COPY --from=builder /home/node/catenax-api/src/database/migrations ./src/database/migrations/
